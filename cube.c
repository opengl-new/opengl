...
*  http://ubuntu.talkera.org
*
*  Compilation trough:
*     gcc cube.c -lglut -lGL -lGLU -lSDL -lSDL_image
*
...
int LoadGLTextures( ) {
    ...
    if ((TextureImage[0] = IMG_Load("fire.jpg"))) {
	    Status = TRUE;
...
void initStars() {
  ...
  for (i = 0; i < NR_STARS; i++) {
    starx[i] = 0.1 * (rand() % 10 + 1);   
    stary[i] = 0.1 * (rand() % 10 + 1);   
    starz[i] = -10;   
...
//    glScalef(scale,scale,scale);
    ...
    for (i = 0; i < NR_STARS; i++) {
     glPushMatrix();
      ...
      // scale particle according to height
      glScalef( stary[i],stary[i],stary[i]);
      drawStar();
      glPopMatrix();
